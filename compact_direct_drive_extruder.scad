...
drive_gear_hobbed_radius = 12 / 2;
drive_gear_hobbed_offset = 3.5;
drive_gear_length = 14;
drive_gear_tooth_depth = (11-9.3)/2;
...
module nema17_mount()
{
	...
	difference()
	{
		...
		// center hole //TS TODO - można zmniejszyść h
		translate([0, 0, -epsilon]	)
			cylinder(r = 11.25 + extra_radius, h = base_height/4 + 2 * epsilon, $fn = 32);

		// axle hole
		color("red") translate([0, 0, -epsilon])
		...
		translate([nema17_hole_offsets[4][0],nema17_hole_offsets[4][1],0])
				cylinder(r = 3.15 + 2.5 * extra_radius, h = 3.5 + 3 * extra_radius, center = true, $fn = 6);
	}
}
...
module filament_tunnel()
{
	...
	translate([0, 0, height / 2])
	{
		difference()
		{
			...
			translate([drive_gear_outer_radius  + filament_diameter / 2, 0, 0])
				cylinder(r = bearing_radius+0.5, h = height + 2 * epsilon, center = true, $fn = 32);
			...
			for (i = [0:4])
		//	hull() {
				translate(nema17_hole_offsets[i] - [filament_offset[0], 0, height / 2 + 1.5])
					sphere(r = m3_head_radius, $fn = 16);

	//			translate(nema17_hole_offsets[i] - [filament_offset[0], 0, height / 2 ])
					sphere(r = m3_head_radius, $fn = 16);
                
	//		}
          // hex key spacing
            for (i = [2:3])
            translate(nema17_hole_offsets[i] - [filament_offset[0], 0, height /1])
            color("red") rotate ([sign(nema17_hole_offsets[i][1])*5,5,0])cylinder (r = m3_head_radius*1, h=30, $fn=32);
...
module bearing_608zz()
{
	difference()
	{
		cylinder(r = bearing_radius, h = bearing_height, center = true, $fn = 32);
		cylinder(r = bearing_inner_radius, h = bearing_height + 2 * epsilon, center = true, $fn = 16);
	}
}
...
module idler_608()
{
	...
	bearing_bottom = filament_offset[2] / 2 - base_height / 2 - 5;
	...
	difference()
	{
		union()
		{
			...
			intersection()
			{
			color("orange") 	cube([width, width, height], center = true);
                
				translate([0, 0, 0])
					cylinder(r = edge_radius, h = height + 2 * epsilon, $fn = 128, center = true);
               color("red") translate([offset + 10.65 + gap, 0, 0])
					cube([15, nema17_width + epsilon, height], center = true);
     
                hull () {
                    color("blue") translate([16, 15.5, 0])
					cylinder(r = edge_radius/5, h = height + 2 * epsilon, $fn = 128, center = true);
                    color("blue") translate([16, -15.5, 0])
					cylinder(r = edge_radius/5, h = height + 2 * epsilon, $fn = 128, center = true);
                }
			}
			...
			translate([offset + bearing_radius - pre_tension, 0, -height / 2])
				cylinder(r = bearing_inner_radius - extra_radius + 1, h = height - .5, $fn = 32);
		...
		translate([offset + bearing_radius - pre_tension, 0, bearing_bottom])
			difference()
			{
				// bearing spare out
				cylinder(r = bearing_radius +0.5, h = 60, $fn = 32);
				
				// bearing mount
				cylinder(r = bearing_inner_radius - extra_radius, h = bearing_height+0.5, $fn = 32);
				
				// bearing mount base
				cylinder(r = bearing_inner_radius - extra_radius + 1, h = 0.5, $fn = 32);
			}

		// bearing mount hole
		translate([offset + bearing_radius - pre_tension, 0, 0])
			cylinder(r = m3_radius, h = 50, center = true, $fn = 32);
        
        translate([offset + bearing_radius - pre_tension, 0, -m3_nut_height-2]) rotate ([0,0,30])
	...
	translate([offset + bearing_radius - pre_tension, 0, filament_offset[2] - base_height])
		%bearing_608zz();
}
